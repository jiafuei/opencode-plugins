  ...
  return {
    ...
    deliver(rawUrl: string): string {
      try {
        ...
        reject(new Error("malformed callback URL"));
        return CALLBACK_FAILURE_HTML;
      }
    },
    dispose() {
      ...
      methods: [
        {
          ...
          authorize: async () => {
            ...
            return {
              ...
              instructions:
                "Open the URL below in your browser and complete the sign-in; the local callback server finishes the login automatically.",
        ...
        },
        {
          ...
          authorize: async () => {
            ...
            return {
              url: buildAuthUrl(state, REDIRECT_URI),
              instructions:
                "Open the URL below, complete the sign-in, then paste either the redirected URL or just the `code` query parameter back into OpenCode.",
