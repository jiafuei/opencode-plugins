...
- If a native compact request fails or returns no usable output, the plugin makes up to three attempts total, waiting one second before the second attempt and two seconds before the third. The assistant request waits until these attempts finish. If all three fail, the current request falls back to the previous usable window or original history, and native compaction is disabled for the rest of that user turn. A new user message or model change allows another attempt.
- Provider rejections are returned normally; a generic `400`/`422` does not invalidate the stored window. State read/write and usage-loading errors propagate instead of being treated as missing data; a missing state file is expected on first use.
...
- API-key compaction follows [OpenAI's documented `POST /v1/responses/compact`](https://platform.openai.com/docs/api-reference/responses/compact): the request carries only `model`, `input` and `instructions`, and the response's `output` array is stored verbatim as the window. ChatGPT OAuth uses Codex's `remote_compaction_v2` SSE protocol and retains recent real user messages alongside its single opaque compaction item. If native compaction is unavailable for your account, the plugin logs a warning and skips further compaction attempts for that user turn.
