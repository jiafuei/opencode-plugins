...
- Dream results apply automatically. While a run is active, its parent session's sidebar shows a `Memory` section with `Dreaming...`; the indicator spans all selector and curator workers and clears on success, no-op, or failure. Manual failures retain their warning toast, while automatic failures remain log-only. Every run writes a decision-only manifest under `.dreams/`, but manifests do not retain old topic contents and therefore cannot provide exact rollback. A separate `.dream.lock` prevents duplicate runs across OpenCode processes without holding the short-lived commit lock during model calls.
