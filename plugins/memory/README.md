...
- Checkpoints capture only previously completed genuine user messages. Assistant output and tool activity are never collected or sent to memory workers, so ordinary memory cannot automatically retain facts inferred from the codebase or the model's own output. When a new real user message arrives and `interval` turns have completed since the last checkpoint, the buffered prompts are snapshotted before the new prompt is buffered — the triggering prompt is never classified. A save classification returns up to three atomic decisions, each a narrow subject describing exactly one thing to remember, and extraction is constrained to that subject. Everything runs in the background and does not delay that user message or later messages.
- An idle timer saves the final pending user messages from short sessions that do not reach a periodic checkpoint. A checkpoint whose classification or extraction fails restores its buffered source unchanged, and that buffer is not offered for review again until another genuine user message is collected.
...
- Maintenance and dreaming share one cleanup engine and the configured dream model. It runs when the managed index exceeds 32 KiB or 200 topics, on `/dream` or `Dream now`, or through the opt-in automatic gate. Complete selected files must fit the 32 KiB input cap, and source contents are rechecked before commit.
...
- Source filenames and revisions are recorded in decision-only run manifests, not as live dependencies in replacement topics. Legacy `sources` metadata does not protect topics from synthesis or cause cascading removal; every prune requires its own verdict.
...
Ordinary learning classifies topics as `preference` (durable general preferences stated by the user), `instruction` (scoped general instructions that apply to future work), `recap` (hard-won completed-task context worth avoiding re-derivation), or `reference` (lasting external material). Synthesis preserves a shared source type; mixed-type groups produce an `insight`, a non-authoritative memory that distinguishes derived conclusions from user-stated facts. A group containing an insight cannot become an instruction or preference. Ordinary extraction cannot create insights. Bodies are concise natural prose with per-type length caps and no mandatory Why/How formatting. Ordinary extraction omits absolute dates from the body — the plugin records timing metadata itself. Existing topics classified as the retired `feedback` and `project` types stay readable.

Topic files carry plugin-owned metadata. `sessionId` is the last writer: the originating session for creation/replacement and the maintenance or dream parent session for synthesis. `scope` names where the memory applies, and `updatedAt` is the plugin-owned ISO write date. Synthesis outputs also carry `dreamRunId`, linking them to the run manifest's source filenames and revisions.
